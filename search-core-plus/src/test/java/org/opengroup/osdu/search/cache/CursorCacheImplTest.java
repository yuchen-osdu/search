...
class CursorCacheImplTest {

    @Mock
    private RedisCache<String, CursorSettings> redisCache;

    private CursorCacheImpl sut;

    @BeforeEach
    void setUp() {
        sut = new CursorCacheImpl(redisCache);
    }

    @Test
    void put_delegatesToRedisCache() {
        CursorSettings settings = mock(CursorSettings.class);
    ...
    void get_delegatesToRedisCache() {
        CursorSettings expected = mock(CursorSettings.class);
        when(redisCache.get("key1")).thenReturn(expected);

        CursorSettings result = sut.get("key1");
